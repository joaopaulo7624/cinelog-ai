...
const App: React.FC = () => {
  ...
  const renderCineLog = () => {
    const heroEntry = filteredEntries.length > 0 ? filteredEntries[0] : null;
    ...
    return (
      <>
        ...
        <header className="sticky top-0 z-40 bg-[#0a0a0a]/90 backdrop-blur-md px-6 py-4 border-b border-white/5">
          <div className="max-w-[1600px] mx-auto flex items-center justify-between">
            ...
            <div className="flex items-center gap-4 relative">
              ...
              <button onClick={() => setIsSettingsOpen(!isSettingsOpen)} className="w-8 h-8 rounded-full bg-gray-700 overflow-hidden border border-white/10 profile-trigger">
        ...
        <main className="max-w-[1600px] mx-auto px-6 pb-12 pt-8">
          {/* Hero CineLog (Dynamic) */}
          {viewMode === 'list' && !searchTerm && heroEntry && (
            <div className="relative w-full h-[400px] rounded-3xl overflow-hidden mb-10 group cursor-pointer shadow-2xl">
              <img
                src={heroEntry.imageUrl || `https://picsum.photos/seed/${heroEntry.id}/1200/600`}
                alt={heroEntry.title}
                className="w-full h-full object-cover object-top transition-transform duration-1000 group-hover:scale-105"
              />
              <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/20 to-transparent" />
              <div className="absolute bottom-0 left-0 p-10 flex flex-col gap-4 max-w-2xl">
                <h2 className="text-5xl font-bold text-white leading-tight drop-shadow-lg line-clamp-2">{heroEntry.title}</h2>
        ...
        <header className="sticky top-0 z-40 bg-[#050505]/90 backdrop-blur-md px-6 py-4 border-b border-purple-500/10 shadow-[0_4px_20px_rgba(168,85,247,0.1)]">
          <div className="max-w-[1600px] mx-auto flex items-center justify-between">
            <div className="flex items-center gap-6">
              ...
              <button
                onClick={() => setAppMode('cine')}
                className="px-4 py-1.5 rounded-full bg-[#1f1f1f] border border-purple-500/20 text-[10px] font-bold text-purple-400 uppercase tracking-wider hover:bg-[#2a2a2a] transition-colors"
              >
                Trocar para Filmes
              </button>
            </div>
            <div className="flex items-center gap-4 relative">
              ...
              <button onClick={() => setIsSettingsOpen(!isSettingsOpen)} className="w-9 h-9 rounded-lg bg-gray-800 border border-purple-500/20 overflow-hidden profile-trigger">
                <img src="https://i.pravatar.cc/150?img=11" alt="User" className="w-full h-full object-cover" />
              </button>
              <SettingsModal
                isOpen={isSettingsOpen}
            ...
            <div className="relative w-full h-[400px] rounded-[32px] overflow-hidden mb-12 group cursor-pointer shadow-[0_0_40px_rgba(168,85,247,0.15)] border border-purple-500/40">
              ...
              <div className="absolute top-0 left-0 bottom-0 p-12 flex flex-col justify-center gap-6 max-w-3xl">
                <div>
                  ...
                  <h2 className="text-6xl font-black text-white leading-none mb-4 line-clamp-2">{heroEntry.title}</h2>
                  <div className="flex gap-2">
                    {heroEntry.platform && <span className="px-3 py-1 rounded bg-white/10 text-xs font-bold text-white border border-white/10">{heroEntry.platform}</span>}
  ...
  return (
    <div className={`min-h-screen ${appMode === 'cine' ? 'bg-[#0a0a0a]' : 'bg-[#050505]'} text-slate-100 pb-24 transition-colors duration-500`}>
      ...
      {/* Settings Modal is now rendered inside the headers for positioning context */}
    </div>
  );
};
